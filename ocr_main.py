...
if __name__ == '__main__':
    ...
    parser.add_argument('--channel', default='kbsn')
    parser.add_argument('--input_video', default='videos/kbsn0707.mp4')
    parser.add_argument('--input_coords', default='input_folder/text/kbsn.txt', help = "Cropping points, crop할 img의 좌표")
    parser.add_argument('--input_folder_postprocess', default='input_folder/words/', help = "post process에 사용할 txt folder")
    parser.add_argument('--input_folder_image', default='input_folder/image/', help= "video -> [[[img]]] -> crop")

    parser.add_argument('--cropimg_folder', default='scoreimgtotext/')
    parser.add_argument('--image_folder', help='path to image_folder which contains text images', default= 'scoreimgtotext/')
    parser.add_argument('--json_path', default="scoretojson.json")


    parser.add_argument('--workers', type=int, help='number of data loading workers', default=4)
    parser.add_argument('--batch_size', type=int, default=192, help='input batch size')
    parser.add_argument('--saved_model', help="path to saved_model to evaluation", default='saved_models/TRBA_33211/best_accuracy.pth')
    """ Data processing """
    ...
    dashed_line = '-' * 70
    head = f'{"team1":10s}\t{"score":10s}\t{"team2":10s}\t{"score":10s}\t{"Pitcher":15s}\tbase'
    log.write(f'{dashed_line}\n{head}\n{dashed_line}\n')
    log.close()


    # 코드 실행 부분
    FrameExtractor.extract_frames(opt)


    with open(opt.json_path, 'a', encoding='utf-8') as file:
        file.write('}')  # 맨 뒤에 '}' 추가